...
class MPCOptimizer:
    ...
    def get_action(self, simulator_state, dt_s=1.0) -> tuple[float, float]:
        ...
        if optimal_u_0 < 0.1:
            # [Fix] Coast on ICE/momentum instead of aggressively braking when not deploying.
            # Lowered the threshold to 0.1 kW to support the EV fleet scenario which has a 5.0 kW max deploy.
            return (0.0, 0.0)
